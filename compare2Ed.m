...
% Calculate the thermocline and hence the MLD
...
for i = 1:num_lats
    for j = 1:num_longs
        ...
        for day = 1:num_days
            ...
            % depth = min(find(temp_grad>tol)); %%%%% UNCOMMENT
            [depth, ~] = MLD(day-1, params);
            % The row
            % depths(day,i,j)=depth;  %%%%% UNCOMMENT
            % The actual depth in meters
            % depths_LEVs(day,i,j)=hawaii_soest_7e38_7a7b_afxhffc2.LEV(depth);  %%%%% UNCOMMENT
            depths_LEVs(day,i,j)= depth;
            % Find average temperature within the mixed layer (degrees
            % Celsius)
            % temps_av(day,i,j)=squeeze(mean(temps(day,1:depth,i,j),2)); %%%%% UNCOMMENT
            temps_av(day,i,j)=squeeze(mean(temps(day,1:7,i,j),2));
        end
        if params.smooth
...
for i = 1:num_lats
    for j = 1:num_longs
        ...
        % covered
        params.area_grid_site_ij = earth_SA * ...
            areaquad(lats(i,1),longs(j,1),lats(i,2),longs(j,2));
        params.volumes(i,j,:)=params.LEV_ij.*params.area_grid_site_ij;
...
function dxdt = odefunc(t,x,params)
    ...
    if params.change_mix_depth == 1
        ...
        N = N/(LEV*params.area_grid_site_ij);%params.vols_ij(1);
        P = P/(LEV*params.area_grid_site_ij);%params.vols_ij(1);
        Z = Z/(LEV*params.area_grid_site_ij);%params.vols_ij(1);
        if params.detritus_layer
            D = D/(LEV*params.area_grid_site_ij);%params.vols_ij(1);
        end
    ...
    elseif params.change_mix_depth == 2
        % a, k, s and psi need to be updated with the new depth
        a = a*12.5/LEV;
        k = k*12.5/LEV;%params.LEV_ij(1)/LEV; %12.5/LEV;
        s = s*12.5/LEV;%params.LEV_ij(1)/LEV;
    ...
    if params.change_mix_depth == 1
        dNdt = params.area_grid_site_ij * LEV * dNdt ...
            + h * params.area_grid_site_ij * ...
            (N_0*heaviside(h) + N*heaviside(-h));
        dPdt = params.area_grid_site_ij * LEV * dPdt ...
            + h * params.area_grid_site_ij * P * heaviside(-h);
        dZdt = params.area_grid_site_ij * LEV * dZdt;
        if params.detritus_layer
            dDdt = params.area_grid_site_ij * LEV * dDdt ...
                + h * params.area_grid_site_ij * ...
                (D*heaviside(-h));
        end
    elseif params.change_mix_depth == 2
        dNdt = dNdt + max(h,0) * (N_0-N)/LEV;
        dPdt = dPdt - max(h,0) * P/LEV;
        dZdt = dZdt - h*Z/LEV;
        if params.detritus_layer
            dDdt = dDdt - max(h,0) * D/LEV;
        end
    end
